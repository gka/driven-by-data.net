<script lang="ts">
	...
	let { posts, activeTag = null, activePublication=null}: { posts: Post[], activeTag: string | null, activePublication: string|null } = $props();
	...
	const groupedPosts = $derived(
		typeof activeTag === 'string'
            ...
            typeof activePublication === 'string' ? 
            [[`Published at ${nicePubNames[activePublication] || activePublication}`, filteredPosts]]
			: Object.entries(
			...
			<div class="column is-3">
				...
				{#each publications as pub}
					<a
                        ...
                        href="/publication/{pub.toLowerCase()}"
						>{nicePubNames[pub] ?? pub}</a
					>
				{/each}
			</div>
			<!-- filter by year -->
